...
export function JobPage({ deleteJob }: JobPageProps) {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const job = useLoaderData() as Job;
  const onDeleteClick = (jobId: string) => {
    const confirm = window.confirm("Are you sure u wanna delete this job?");
    if (!confirm) return;
    deleteJob(jobId);
...
// Use LoaderFunctionArgs to type the incoming arguments safely without 'any'
